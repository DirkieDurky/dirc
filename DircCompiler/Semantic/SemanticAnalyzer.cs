...
public class SemanticAnalyzer
{
    ...
    public void Analyze(List<AstNode> nodes, CompilerOptions options, CompilerContext context)
    {
        ...
        foreach (var node in nodes)
        {
            ...
            {
                var signature = new FunctionSignature(func.ReturnTypeToken.Lexeme, func.Parameters);
    ...
    private string? AnalyzeNode(AstNode node, string? expectedType, CompilerOptions options, CompilerContext context)
    {
        switch (node)
        {
            case VariableDeclarationNode varDecl:
                ...
                {
                    var initType = AnalyzeNode(varDecl.Initializer, varType, options, context);
            ...
            case VariableAssignmentNode varAssign:
                if (!_variables.TryGetValue(varAssign.Name, out var assignType))
                {
                ...
                {
                    var valueType = AnalyzeNode(varAssign.Value, assignType, options, context);
            ...
            case IdentifierNode id:
                if (!_variables.TryGetValue(id.Name, out var idType))
                {
            ...
            case ConditionNode cond:
                var leftType = AnalyzeNode(cond.Left, "int", options, context);
                var rightType = AnalyzeNode(cond.Right, "int", options, context);
                if (leftType != "int" || rightType != "int")
                {
                    throw new SemanticException($"Condition operands must be int, got {leftType} and {rightType}", null, options, context);
                }
                return "bool";
            case IfStatementNode ifStmt:
                var condType = AnalyzeNode(ifStmt.Condition, "bool", options, context);
                if (condType != "bool")
                {
                    throw new SemanticException($"If condition must be bool, got {condType}", null, options, context);
                }
                foreach (var stmt in ifStmt.Body) AnalyzeNode(stmt, null, options, context);
                if (ifStmt.ElseBody != null) foreach (var stmt in ifStmt.ElseBody) AnalyzeNode(stmt, null, options, context);
                return null;
            case WhileStatementNode whileStmt:
                var whileCondType = AnalyzeNode(whileStmt.Condition, "bool", options, context);
                if (whileCondType != "bool")
                {
                    throw new SemanticException($"While condition must be bool, got {whileCondType}", null, options, context);
                }
                foreach (var stmt in whileStmt.Body) AnalyzeNode(stmt, null, options, context);
                return null;
            case CallExpressionNode call:
                if (!_functions.TryGetValue(call.Callee, out var sig))
                {
                ...
                {
                    var argType = AnalyzeNode(call.Arguments[i], sig.Parameters[i].TypeName, options, context);
            ...
            case FunctionDeclarationNode func:
                ...
                var oldVars = new Dictionary<string, string>(_variables);
                ...
                _variables.Clear();
                foreach (var kv in oldVars) _variables[kv.Key] = kv.Value;
                return null;
            case ReturnStatementNode ret:
                var retType = AnalyzeNode(ret.ReturnValue, expectedType, options, context);
